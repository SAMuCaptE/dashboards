  ...
  return (
    <>
      ...
      <ul class="w-[95%] mx-auto">
        ...
        >
          {(task) => <Task task={task} subtasks={subtasks()[task.id] ?? []} />}
        </For>
      </ul>
    </>
  );
...
const Task = (props: {
  ...
  return (
    <>
      <li
        class="even:bg-gray-200 py-[2px]"
        classList={{ "pl-4": props.offset }}
      >
        <a href={props.task.url} target="_blank">
          <div class="grid grid-cols-[80px_1fr_120px_70px_70px] items-center">
            ...
            <ul class="flex flex-wrap justify-evenly w-24">
              <For
                each={props.task.assignees.sort((a, b) =>
                  a.username
                    .split(" ")[1]
            ...
            <div class="h-6 text-sm text-center">
              {props.task.time_estimate ? (
                formatTime(props.task.time_estimate)
              ) : (
                <Dash />
              )}
            </div>
            ...
            <div class="h-6 text-sm text-center">
              {props.task.time_spent ? (
                formatTime(props.task.time_spent)
              ) : (
                <Dash />
              )}
            </div>
      ...
      <Show when={props.subtasks.length > 0 && showSubtasks()}>
        <For each={props.subtasks}>
          {(subtask) => <Task task={subtask} subtasks={[]} offset={true} />}
        </For>
      </Show>
    </>
  );
