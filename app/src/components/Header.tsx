...
const Header: Component = () => {
  const startDate = () => new Date(params.dueDate().getTime() - 6 * 24 * 3600 * 1000);
  ...
  return (
    <header class="h-[1in] w-11/12 mx-auto my-0 grid grid-cols-[2fr_1fr] items-center relative">
      <hgroup>
        ...
        <h3>
          Période du <strong>{dateFormatter.format(startDate())}</strong> au <strong>{dateFormatter.format(params.dueDate())}</strong>
        </h3>
      </hgroup>
